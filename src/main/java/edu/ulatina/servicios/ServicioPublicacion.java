...
public class ServicioPublicacion extends HibernateUtil {
    ...
    public List< Publicacion> Allpublicacions() {
        List< Publicacion> daoAllpublicacions = new ArrayList<>();
        try {
            String jpql = "SELECT t FROM " + Publicacion.class.getSimpleName() + " t";
            daoAllpublicacions = this.getEntityManager().createQuery(jpql, Publicacion.class).getResultList();

        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("Error*");
        }
        return daoAllpublicacions;
    }
